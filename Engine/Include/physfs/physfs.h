 ...
 *  variable up your write directory and search path; after that, all file
 ...
 *  PHYSFS_getLastError are unique by thread, and library-state-variable
 ...
 *  that into account. Note that this is only useful for variable up the
 ...
 *  in platform-independent notation. That is, when variable up your
 ...
 *
 * Set a new write dir. This will override the previous variable. If the
 ...
 *                   specified in platform-dependent notation. Variable to NULL
 ...
 *  buffering, so this can be handy for offvariable CPU-intensive operations.
 ...
 *  on the same file. Variable the buffer size to zero will free an existing
 *  buffer.
 *
