...
void BSPLoader::ReadEntities(std::ifstream & file) {
	delete entities;
	entities = new char[header.entries[BSPLumpEntities].length + 1];
	
	file.seekg(header.entries[BSPLumpEntities].offset, std::ios::beg);
	if (!file.read(reinterpret_cast<char *>(entities), header.entries[BSPLumpEntities].length))
		throw TGen::RuntimeException("BSPLoader::ReadEntities", "failed to read entities");
	
	entities[header.entries[BSPLumpEntities].length] = 0;
	
	
	std::cout << "[bsp]: entities: " << entities << std::endl;
}
...
void BSPLoader::CreateNodes(TGen::Renderer & renderer, BSPNode * node, int nodeIndex, SurfaceLinker & linker) {
	...
	Plane * plane = &planes[bspNode->plane];
	...
	std::cout << "----> " << node->plane.normal.x << ", " << node->plane.normal.y << ", " << node->plane.normal.z << " " << node->plane.distance << std::endl;
	...
	if (bspNode->children[0] >= 0) {
		...
		node->leaf1->index = -(bspNode->children[0] + 1);
	...
	if (bspNode->children[1] >= 0) {
		...
		node->leaf2->index = -(bspNode->children[1] + 1);
...
void BSPLoader::CreateLeaf(TGen::Renderer & renderer, BSPLeaf * leaf, BSPNode * node, int leafIndex, SurfaceLinker & linker) {
	...
	for (int i = bspLeaf->leafface; i < bspLeaf->leafface + bspLeaf->n_leaffaces; ++i) {
		...
		//if (facesAdded.find(leafFace->face) == facesAdded.end()) {
			CreateFace(renderer, leaf, node, leafFace->face, linker);
		//	facesAdded[leafFace->face] = true;
		//}
...
void BSPLoader::CreateFace(TGen::Renderer & renderer, BSPLeaf * leaf, BSPNode * node, int faceIndex, SurfaceLinker & linker) {
	...
	leaf->AddSurface(Surface(linker.getMaterial(textures[bspFace->texture].name), loadedFaces[faceIndex]));			


}

void BSPLoader::CreateFaces(TGen::Renderer & renderer, BSPNode * node) {
	delete [] loadedFaces;
	
	loadedFaces = new BSPGeometry*[numFaces];
	
	for (int i = 0; i < numFaces; ++i) {
		Face * bspFace = &faces[i];
...
BSPTree * BSPLoader::CreateTree(TGen::Renderer & renderer, SurfaceLinker & linker) {
	...
	CreateFaces(renderer, tree->root);	
